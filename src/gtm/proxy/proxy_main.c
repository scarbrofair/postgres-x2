...
static int GTMProxy_HandshakeConnection(GTMProxy_ConnectionInfo *conninfo);
...
GTMProxy_ThreadMain(void *argp)
{
	...
	for (;;)
	{
		...
		if (!first_turn)
		{
			...
			{
				Enable_Longjmp();
				nrfds = epoll_wait(thrinfo->thr_epoll_fd, event_set,
									GTM_PROXY_MAX_CONNECTIONS, 1000);
				Disable_Longjmp();
...
setjmp_again:
		...
		for (ii = 0; ii < nrfds; ii++)
		{
			...
			if (conninfo ==NULL) {
				elog(FATAL, "Failed to find conninfo for socket %d", event_set[ii].data.fd);
			}
...
GTMProxy_HandshakeConnection(GTMProxy_ConnectionInfo *conninfo)
{
	...
	if (pq_flush(conninfo->con_port) != 0) {
		return STATUS_ERROR;
	}
...
GTMProxy_HandleDisconnect(GTMProxy_ConnectionInfo *conninfo, GTM_Conn *gtm_conn)
{
	...
	if (gtmpqPutMsgStart('C', true, gtm_conn) ||
		...
		elog(WARNING, "Failed to proxy data");
		goto Disconnect_end;
	}
	...
	/* Finish the message. */
	if (gtmpqPutMsgEnd(gtm_conn)) {
		elog(WARNING, "Failed to finish the message");
	}
